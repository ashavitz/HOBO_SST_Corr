...
summary(temp_lm_1)
temp_lm_2 <- lm(gdd.hobo.3 ~ gdd.leol3s_acspo.3, data = temp_data_daily_2025)
summary(temp_lm_2)
temp_lm_3 <- lm(gdd.hobo.3 ~ gdd.crw.3, data = temp_data_daily_2025)
summary(temp_lm_3)

# Factor only depth into models
temp_lm_d1 <- lm(gdd.hobo.3 ~ gdd.jpl.3 + depth_m, data = temp_data_daily_2025_MA)
summary(temp_lm_d1)
temp_lm_d2 <- lm(gdd.hobo.3 ~ gdd.leol3s_acspo.3 + depth_m, data = temp_data_daily_2025_MA)
summary(temp_lm_d2)
temp_lm_d3 <- lm(gdd.hobo.3 ~ gdd.crw.3 + depth_m, data = temp_data_daily_2025_MA)
summary(temp_lm_d3)

# Factor only tidal exchange into models
temp_lm_t1 <- lm(gdd.hobo.3 ~ gdd.jpl.3 + tidal_range_FT, data = temp_data_daily_2025_t)
summary(temp_lm_t1)
temp_lm_t2 <- lm(gdd.hobo.3 ~ gdd.leol3s_acspo.3 + tidal_range_FT, data = temp_data_daily_2025_t)
summary(temp_lm_t2)
temp_lm_t3 <- lm(gdd.hobo.3 ~ gdd.crw.3 + tidal_range_FT, data = temp_data_daily_2025_t)
summary(temp_lm_t3)
...
ggplot(temp_data_daily_2025_t, aes(x = gdd_hobo_pred, y = gdd.hobo.3)) +
  geom_point(color = "grey") + 
  geom_smooth(method = "lm", color = "red") +
  # Annotate plot with simple linear model p-values and R2 values
  stat_poly_eq(use_label("eq", "R2", "p"))
...
plot_data <- temp_data_daily_2025_t |>
  pivot_longer(
    cols = c(gdd.hobo.3, gdd_hobo_pred),
...
label_txt <- paste0(
  "gdd.hobo.3 = ", round(b[1], 3),
  " + ", round(b["gdd.crw.3"], 3), "·gdd.crw.3",
  " + ", round(b["tidal_range_FT"], 3), "·tidal_range_FT",
  " + ", round(b["depth_m"], 3), "·depth_m\n",
  "R² = ", round(s$r.squared, 3),
  " | p(crw) = ", signif(s$coefficients["gdd.crw.3","Pr(>|t|)"], 3),
  " | p(tidal) = ", signif(s$coefficients["tidal_range_FT","Pr(>|t|)"], 3),
  " | p(depth) = ", signif(s$coefficients["depth_m","Pr(>|t|)"], 3)
)

ggplot(temp_data_daily_2025_MA,
       aes(x = gdd.crw.3, y = gdd.hobo.3, color = tidal_range_FT)) +
  geom_point() +
  geom_smooth(method = "lm", color = "red") +
  annotate("text", x = Inf, y = Inf, label = label_txt,
           hjust = 1.05, vjust = 1.2, size = 3.5) +
